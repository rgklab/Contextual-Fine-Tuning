...
def eval_batch(model, task_sampler, base_task_sampler, args, xs, xs_p=None):
    task = task_sampler()
    base_task = base_task_sampler()
    ...
    if xs_p is None:
        ...
        pred = model(xs.to(device), ys.to(device), ys_base.to(device), args, inds=torch.arange(ys.shape[1], ys.shape[1]*2)).detach()
...
def eval_model(
    ...
    for i in range(num_eval_examples // batch_size):
        ...
        metrics = eval_batch(model, task_sampler, base_task_sampler, args, xs, xs_p) 
...
def compute_evals(all_models, evaluation_kwargs, args, save_path=None, recompute=False):
    try:
    ...
    for eval_name, kwargs in tqdm(evaluation_kwargs.items()):
        ...
        for model in all_models:
            ...
            metrics[model.name] = eval_model(model, args, **kwargs)
...
def get_run_metrics(
    ...
    if save_path is not None and os.path.exists(save_path):
        ...
        if checkpoint_created > cache_created:
            recompute = True
    all_metrics = compute_evals(all_models, evaluation_kwargs, args, save_path, recompute)
    return all_metrics
