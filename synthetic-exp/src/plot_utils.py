...
def collect_results(run_dir, df, valid_row=None, rename_eval=None, rename_model=None, args=None, step=None):
    all_metrics = {}
    for _, r in df.iterrows():
        ...
        metrics = get_run_metrics(run_path, args, step=step,  skip_model_load=True, skip_baselines=True)
        ...
        for eval_name, results in sorted(metrics.items()):
            ...
            for model_name, m in results.items():
                ...
                xlim = 2 * n_dims + 1
                if r.task in ['sumdot_2nn_regression', "polynomial_2nn_regression", "relu_2nn_regression", "decision_tree"]:
                    xlim = 200
